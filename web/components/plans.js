...
export function PlanItem({data, layer, editStatus, actions, parents, brother, password, init, loginStatus, css=[], selected=false}){
  ...
  const [ showBody, setShowBody ] = useState(init?.showBody===undefined?((edit||showPublic)?false:true):init.showBody)
  ...
  const [ showContent, setShowContent ] = useState(init?.showContent===undefined?(false||showPublic):init.showContent)
